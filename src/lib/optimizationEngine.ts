...
export class OptimizationEngine {
  ...
  private initializeProviderPerformance() {
    ...
    providers.forEach(provider => {
      this.providerPerformance.set(provider.id, {
        ...
        performance: provider.performance || 0.90,
        lastUsed: new Date()
      });
    });
  }
    ...
    for (const rule of enabledRules) {
      try {
        if (rule.condition(this.metrics)) {
          const decision = rule.action(this.metrics);
          if (decision) {
  ...
  private getLeastUsedProvider(): ProviderPerformance {
    const performances = Array.from(this.providerPerformance.values());
    return performances.reduce((least, current) => 
  ...
  getOptimizationRecommendations(): string[] {
    ...
    recommendations.push(`Consider switching to ${cheapestProvider.providerName} for ${((cheapestProvider.currentCost / prices[0].currentCost - 1) * 100).toFixed(1)}% cost savings`);
    
    // Performance recommendations
    const bestPerformingProvider = performances.reduce((best, current) => 
      current.reliability > best.reliability ? current : best
    );
    recommendations.push(`Switch to ${bestPerformingProvider.providerName} for better reliability (${(bestPerformingProvider.reliability * 100).toFixed(1)}%)`);
