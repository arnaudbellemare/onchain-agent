...
export async function POST(req: Request) {
  try {
    ...
    if (userMessage.includes("payroll") || userMessage.includes("salary")) {
      ...
      } else if (userMessage.includes("demo") || userMessage.includes("show")) {
        // Show x402 demo
        response = `🔗 **x402 Protocol Live Demo:**\n\n🎯 **Available API Providers:**\n• **CoinGecko:** Bitcoin/Ethereum prices ($0.005 per call)\n• **AIxbt:** Bitcoin AI analysis ($0.02 per call)\n• **OpenAI:** GPT-4 completions ($0.03 per call)\n• **Weather API:** Current weather ($0.001 per call)\n\n💡 **Try These Commands:**\n• "Get Bitcoin price" - Real-time price with x402 payment\n• "Get Bitcoin analysis" - AI-powered analysis with x402 payment\n• "Process x402 API payment" - Process a generic x402 payment\n• "Show x402 status" - View current x402 API status\n\n🚀 **x402 Benefits Demonstrated:**\n• ✅ Micropayments: Pay only for what you use\n• ✅ Instant Access: No account creation required\n• ✅ Autonomous: AI agents can make payments\n• ✅ Cost Savings: 30% cheaper than traditional APIs\n\n**Ready to test x402 protocol?** Try any of the commands above!`;
      } else if (userMessage.includes("use cases") || userMessage.includes("examples")) {
        // Show x402 use cases
        response = `🚀 **x402 Protocol Use Cases - AI Agents & Humans:**\n\n🤖 **AI Agents Accessing APIs:**\n• **Research Platform:** AI pays $0.25 per premium article (vs $50/month subscription)\n• **Video Streaming:** AI pays $0.001 per second watched (vs monthly subscriptions)\n• **Trading AI:** Pays $0.02 per market data request (vs expensive real-time feeds)\n\n🧠 **Pay-Per-Use AI Model Inference:**\n• **Computer Vision:** $0.005 per image classification (vs fixed enterprise fees)\n• **Synthetic Voice:** $0.10 per audio clip generation (flexible monetization)\n\n☁️ **Agents Paying for Cloud Compute:**\n• **GPU Resources:** $0.50 per GPU-minute (no upfront infrastructure)\n• **Cloud Storage:** $0.01 per GB stored (dynamic scaling)\n\n📋 **Contract Retrieval for Agents:**\n• **Financial AI:** $0.25 per premium news article (vs full subscriptions)\n• **Legal Research:** $0.10 per court ruling (vs expensive database access)\n\n👥 **Micropayments for Human Content:**\n• **Substack Writer:** $0.25 per article (vs $10/month subscription)\n• **Research Journal:** $2.00 per whitepaper (vs $200/year membership)\n• **Premium Podcast:** $0.50 per episode (vs $5/month subscription)\n• **Pay-Per-Play Game:** $0.10 per play (vs $20 purchase or ads)\n\n💡 **Key Benefits:**\n• ✅ Pay only for what you use\n• ✅ No subscription commitments\n• ✅ Lower barriers to premium content\n• ✅ Autonomous AI resource purchasing\n• ✅ Seamless integration with MCP protocol\n\n**Ready to see these in action?** Check out the x402 Use Cases Showcase in the AI Dashboard!`;
      } else {
        // Show x402 status
        const apiCosts = paymentDB.getAPICosts();
        const x402APIs = apiCosts.filter(api => api.x402Enabled);
